...
public class MixUtils {
    ...
    public static String getZkAddress() {
        String zkAddress = System.getenv(Constants.REGISTRY_URL_KEY.replace('.', '_'));
        if (zkAddress == null) {
            zkAddress = System.getProperty(Constants.REGISTRY_URL_KEY);
        }
        if (zkAddress == null) {
            zkAddress = Constants.DEFAULT_REGISTRY_URL;
            logger.error("zookeeper address not found. use default zookeeper address: {}", Constants.DEFAULT_REGISTRY_URL);
        } else {
